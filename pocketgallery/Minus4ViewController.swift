...
class Minus4ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dataSource = AppDelegate.instance.dataSource
               ...
               self.viewModel.fetchData { arr in
                   self.arr = arr
                   self.work = self.arr[currentArtNum-4]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
          if segue.identifier == "4Forwards"{
              let dest = segue.destination as? Minus3ViewController;}
        if segue.identifier == "4Backwards" {
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == "4Backwards"{
            if (currentArtNum-5 > -1){
                return true
            }
            else{
                return false
