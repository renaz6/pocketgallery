...
class YesterdayViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.arr = arr
        self.work = self.arr[currentArtNum-1]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "yesterdayForwards"{
            let dest = segue.destination as? HomeViewController;}
        if segue.identifier == "yesterdayBackwards" {
            print("here")
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == "yesterdayBackwards"{
            if (currentArtNum-2 > -1){
                return true
            }
            else{
                return false
