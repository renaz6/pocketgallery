...
class Minus3ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.viewModel.fetchData { arr in
            self.arr = arr
            self.work = self.arr[currentArtNum-3]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "minus3Forwards"{
            let dest = segue.destination as? Minus2ViewController;}
        if segue.identifier == "3Backwards" {
            let dest = segue.destination as? Minus4ViewController;
        }
            
    }
    
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == "3Backwards"{
            if (currentArtNum-4 > -1){
                return true
            }
            else{
                return false
