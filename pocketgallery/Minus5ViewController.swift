...
class Minus5ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dataSource = AppDelegate.instance.dataSource
                ...
                self.viewModel.fetchData { arr in
                    self.arr = arr
                    self.work = self.arr[currentArtNum-5]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
             if segue.identifier == "5Forwards"{
                 let dest = segue.destination as? Minus4ViewController;}
        
                if segue.identifier == "5Backwards" {
                    let dest = segue.destination as? Minus6ViewController;
                }
                 
         }
    
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == "5Backwards"{
            if (currentArtNum-6 > -1){
                return true
            }
            else{
                return false
